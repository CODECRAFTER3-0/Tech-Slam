...
function DefectDetector({ onBack }) {
  ...
  const saveToHistory = async (imageDataUrl, fileName, timestamp, report = null) => {
    try {
      const newEntry = {
        id: Date.now(),
  ...
  const loadFromHistory = (historyItem) => {
    ...
    setReportData(historyItem.report || null);
  ...
  const getVerdictStyle = (verdict) => {
    switch (verdict) {
      case 'PASS':
        return { color: '#10b981', bg: 'rgba(16,185,129,0.12)', icon: '✅', label: 'PASS — Safe to Use' };
      case 'USABLE WITH CAUTION':
        return { color: '#f59e0b', bg: 'rgba(245,158,11,0.12)', icon: '⚠️', label: 'USABLE WITH CAUTION' };
      case 'REJECT':
        return { color: '#ef4444', bg: 'rgba(239,68,68,0.12)', icon: '❌', label: 'REJECT — Do Not Use' };
      default:
        return { color: '#6b7280', bg: 'rgba(107,114,128,0.12)', icon: '❓', label: verdict };
    }
  };
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      await saveToHistory(imageUrl, fileName || data.image[0].name, new Date().toISOString(), report);
    ...
    <div className="detector-container">
      ...
      <div className="detector-card">
        <div className="detector-header">
          ...
          <h1 className="detector-title">Metal Coil Defect Detector</h1>
        ...
        {resultImg && !loading && !showComparison && (
          <div className="report-container">
            <h3 className="report-main-title">📋 Defect Analysis Report</h3>
            ...
            {reportData && (
              <div
                ...
                style={{
                  background: getVerdictStyle(reportData.verdict).bg,
                  borderLeft: `4px solid ${getVerdictStyle(reportData.verdict).color}`
                }}
              >
                <span className="verdict-icon">{getVerdictStyle(reportData.verdict).icon}</span>
                <div className="verdict-text">
                  <span className="verdict-label" style={{ color: getVerdictStyle(reportData.verdict).color }}>
                    {getVerdictStyle(reportData.verdict).label}
                  </span>
                  <span className="verdict-reason">{reportData.verdict_reason}</span>
                </div>
              </div>
            )}
            ...
            {reportData && (
              <div className="report-stats">
                <div className="stat-card">
                  ...
                  <span className="stat-value">{reportData.total_defects}</span>
                  <span className="stat-label">Defects Found</span>
                </div>
                <div className="stat-card">
                  <span className="stat-icon">⚡</span>
                  <span className="stat-value" style={{ color: getSeverityColor(reportData.severity) }}>
                    {reportData.severity}
                  </span>
                ...
                <div className="stat-card">
                  ...
                  <span className="stat-value">
                    {reportData.image_dimensions?.width}×{reportData.image_dimensions?.height}
                  </span>
                ...
                <div className="stat-card">
                  ...
                  <span className="stat-value">
                    {reportData.defects.length > 0
                      ? `${(Math.max(...reportData.defects.map(d => d.confidence)) * 100).toFixed(1)}%`
                      : 'N/A'}
                  </span>
            ...
            <div className="report-image-section">
              <h4 className="report-section-title">Annotated Image</h4>
              <img src={resultImg} alt="Detection Result" className="result-image" />
            </div>

            {/* Defect Details Table */}
            {reportData && reportData.defects.length > 0 && (
              <div className="report-defects-section">
                <h4 className="report-section-title">Defect Details</h4>
                <div className="defects-table-wrapper">
                  <table className="defects-table">
                    ...
                    <tbody>
                      {reportData.defects.map((defect, idx) => (
                        <tr key={idx}>
                          <td>{idx + 1}</td>
                          <td>
                            <span className="defect-class-badge">{defect.class}</span>
                          </td>
                          <td>
                            <div className="confidence-bar-wrapper">
                              <div
                                ...
                                style={{
                                  ...
                                  background: defect.confidence >= 0.85 ? '#ef4444'
                                    : defect.confidence >= 0.5 ? '#f59e0b' : '#3b82f6'
                                }}
                              />
                              <span className="confidence-text">{(defect.confidence * 100).toFixed(1)}%</span>
                            </div>
                          </td>
                          <td className="bbox-cell">
                            [{defect.bbox.map(v => Math.round(v)).join(', ')}]
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            {reportData && reportData.defects.length === 0 && (
              <div className="no-defects-message">
                ...
                <p>The metal sheet appears to be in excellent condition and is safe for production use.</p>
              </div>
            )}
          </div>
        )}
